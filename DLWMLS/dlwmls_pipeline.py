...
from DLWMLS.utils import prepare_data_folder, rename_and_copy_files

warnings.simplefilter(action="ignore", category=FutureWarning)
warnings.simplefilter(action="ignore", category=UserWarning)


def run_dlwmls_pipeline(
    in_dir: str,
    out_dir: str,
    device: str,
    verbose: bool = False,
    ...
    if clear_cache:
        ...
        if not in_dir or not out_dir:
            logging.info("Cache cleared and missing either -i / -o. Exiting.")
            sys.exit(0)
    ...
    if os.path.isdir(in_dir):  # if args.i is a directory
        ...
        logging.info("Input directory not found. Exiting DLWMLS.")
        sys.exit()
    ...
    if not os.path.exists(model_folder):
        # HF download model
        logging.info("DLWMLS model not found, downloading...")
        ...
        logging.info("DLWMLS model has been successfully downloaded!")
    else:
